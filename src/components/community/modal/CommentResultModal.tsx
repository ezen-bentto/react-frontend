...
const CommentResultModal = ({ type, message, onClose }: ResultModalProps) => {
  if (!type) return null;
    ...
    >
      <div className="modal-box border border-gray-200 dark:border-gray-600 rounded-2xl shadow-2xl p-8 max-w-md">
        <div className="text-center">
          ...
          >
            <span className="text-white text-2xl">{isSuccess ? "✓" : "✕"}</span>
          </div>
          <h3
          ...
          <button
            className={`w-full py-3 text-white rounded-xl transition-all duration-200 font-medium shadow-lg hover:shadow-xl ${
              isSuccess
...
export default CommentResultModal;
